...
class NovelCreationFlowManager:
    ...
    async def process_message(self, session_id: str, user_input: str) -> FlowResponse:
        ...
        # 创建流程
            ...
            CreationFlowStep.WORLD_SETTING_DETAIL,
            CreationFlowStep.WORLD_SETTING_CLARIFY,
        ]:
            return await self._handle_world_setting_step(
                session_id, user_input, context
            )
        elif current_step in [
            CreationFlowStep.SYNOPSIS_EXTRACTION,
            CreationFlowStep.SYNOPSIS_REFINEMENT,
        ]:
        ...
        # 修改流程
            ...
            return await self._handle_revision_target_selection(
                session_id, user_input, context
            )
        elif current_step == CreationFlowStep.REVISION_DETAIL_COLLECTION:
            return await self._handle_revision_detail_collection(
                session_id, user_input, context
            )
        elif current_step == CreationFlowStep.REVISION_CONFIRMATION:
            return await self._handle_revision_confirmation(
                session_id, user_input, context
            )
        else:
            return FlowResponse(
...
您可以简单描述，我会帮您整理成专业的简介！"""
            ...
            # 需要追问
            missing_elements = self._get_missing_world_setting_elements(
...
祝您创作愉快！有任何需要随时找我！✨"""
            ...
            # 用户想修改某些内容
            response_message = await self._handle_modification_request(
                user_input, context
            )
    ...
    async def _get_or_create_context(self, session_id: str) -> NovelCreationContext:
        ...
        if flow:
            context = NovelCreationContext(
                scene=(
                    NovelDialogueScene(flow.scene)
                    if flow.scene
                    else NovelDialogueScene.CREATE
                ),
                current_step=CreationFlowStep(flow.current_step),
                genre=flow.genre,
                world_setting=(
                    WorldSettingDetails(**flow.world_setting_data)
                    if flow.world_setting_data
                    else None
                ),
                synopsis=(
                    NovelSynopsis(**flow.synopsis_data) if flow.synopsis_data else None
                ),
    ...
    async def _save_flow_state(self, session_id: str, context: NovelCreationContext):
        ...
        if flow:
            ...
            flow.world_setting_data = (
                context.world_setting.dict() if context.world_setting else {}
            )
    ...
    "specific_request": "具体查询要求"
        ...
        try:
            ...
            if query_target == "world_setting":
                result = await query_service.get_world_setting(
                ...
                result = await query_service.get_characters(
                    context.selected_novel_id, role_type
                )
                ...
                if "第" in user_input and "章" in user_input:
                    ...
                    if match:
                        ...
                        result = await query_service.get_chapter_content(
                            context.selected_novel_id, chapter_num
                        )
                        result_text = self._format_chapter_content_display(result)
                    else:
                        result = await query_service.get_chapter_list(
                            context.selected_novel_id
                        )
