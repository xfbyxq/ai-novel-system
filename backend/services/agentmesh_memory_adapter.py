...
class NovelMemoryStorage:
    ...
    def _init_tables(self):
        ...
        with self._init_lock:
            ...
            conn.execute('CREATE INDEX IF NOT EXISTS idx_character_novel ON character_states(novel_id)')
            conn.execute('CREATE INDEX IF NOT EXISTS idx_foreshadowing_novel ON foreshadowing(novel_id)')
    ...
    def save_chapter_summary(
        ...
        if existing:
            ...
            logger.debug(f"Updated chapter {chapter_number} summary for novel {novel_id}")
        else:
            # 插入
            ...
            logger.debug(f"Saved new chapter {chapter_number} summary for novel {novel_id}")
    ...
    def get_recent_chapter_summaries(
        ...
        # 返回按章节号升序排列
        return sorted([self._row_to_summary_dict(row) for row in rows],
    ...
    def _row_to_summary_dict(self, row: sqlite3.Row) -> Dict[str, Any]:
        ...
        return {
            ...
            'foreshadowing': json.loads(row['foreshadowing']) if row['foreshadowing'] else [],
    ...
    def _format_summary_for_search(self, summary: Dict[str, Any], chapter_number: int) -> str:
        """格式化摘要用于全文搜索"""
    ...
    def save_character_state(
        ...
        if existing:
            ...
            if existing['state_hash'] == state_hash:
                logger.debug(f"Character {character_name} state unchanged, skipping update")
                return existing['id']
            ...
            record_id = existing['id']
            logger.debug(f"Updated character {character_name} state for novel {novel_id}")
        else:
            # 插入
            record_id = str(uuid.uuid4())[:12]
            conn.execute('''
            ...
            logger.debug(f"Saved new character {character_name} state for novel {novel_id}")
    ...
    def get_all_character_states(self, novel_id: str) -> Dict[str, Dict[str, Any]]:
        ...
        conn = self._get_connection()
        rows = conn.execute(
            'SELECT * FROM character_states WHERE novel_id = ?',
            (novel_id,)
        ).fetchall()

        return {
            row['character_name']: self._row_to_character_dict(row)
            for row in rows
        }
    ...
    def _row_to_character_dict(self, row: sqlite3.Row) -> Dict[str, Any]:
        ...
        return {
            ...
            'pending_events': json.loads(row['pending_events']) if row['pending_events'] else [],
            'updated_at': row['updated_at']
        }

    # ==================== 小说元数据操作（长期记忆） ====================

    def save_novel_metadata(
        self,
        novel_id: str,
    ...
    def get_novel_metadata(self, novel_id: str) -> Optional[Dict[str, Any]]:
        ...
        conn = self._get_connection()
        row = conn.execute(
            'SELECT * FROM novel_metadata WHERE novel_id = ?',
            (novel_id,)
        ).fetchone()
        ...
        if row:
            return {
                ...
                'world_setting': json.loads(row['world_setting']) if row['world_setting'] else {},
                'characters': json.loads(row['characters']) if row['characters'] else [],
                'plot_outline': json.loads(row['plot_outline']) if row['plot_outline'] else {},
                'updated_at': row['updated_at']
            }
        return None
    ...
    def get_statistics(self, novel_id: str) -> Dict[str, Any]:
        ...
        chapter_count = conn.execute(
            'SELECT COUNT(*) FROM chapter_summaries WHERE novel_id = ?',
            (novel_id,)
        ).fetchone()[0]

        character_count = conn.execute(
            'SELECT COUNT(*) FROM character_states WHERE novel_id = ?',
            (novel_id,)
        ).fetchone()[0]
        ...
        chunk_count = conn.execute(
            'SELECT COUNT(*) FROM memory_chunks WHERE novel_id = ?',
            (novel_id,)
        ).fetchone()[0]
...
class NovelMemoryAdapter:
    ...
    async def initialize_novel_memory(
        self,
        novel_id: str,
    ...
    async def update_character_state(
        ...
        new_state = {
            ...
            'current_location': updates.get('current_location', current.get('current_location', '')),
            'cultivation_level': updates.get('cultivation_level', current.get('cultivation_level', '')),
            'emotional_state': updates.get('emotional_state', current.get('emotional_state', '')),
            'relationships': {**current.get('relationships', {}), **updates.get('relationships', {})},
            'status': updates.get('status', current.get('status', 'active')),
            'pending_events': updates.get('pending_events', current.get('pending_events', []))
        }
