...
def upgrade() -> None:
    ...
    op.alter_column(
        'novels', 'status',
        type_=sa.String(50),
        existing_type=postgresql.ENUM('planning', 'writing', 'completed', 'published', name='novelstatus'),
    ...
    op.alter_column(
        'novels', 'length_type',
        type_=sa.String(50),
        existing_type=postgresql.ENUM('short', 'medium', 'long', name='novellengthtype'),
    ...
    op.alter_column(
        ...
        type_=sa.String(50),
        existing_type=postgresql.ENUM('planning', 'writing', 'editing', 'batch_writing', name='tasktype'),
        existing_nullable=False,
    ...
    op.alter_column(
        ...
        type_=sa.String(50),
        existing_type=postgresql.ENUM('protagonist', 'supporting', 'antagonist', 'minor', name='roletype'),
        existing_nullable=True,
    ...
    op.alter_column(
        'characters', 'status',
        type_=sa.String(50),
        existing_type=postgresql.ENUM('alive', 'dead', 'unknown', name='characterstatus'),
        existing_nullable=True,
        postgresql_using='status::text'
    )
    ...
    op.alter_column(
        'chapters', 'status',
        type_=sa.String(50),
        existing_type=postgresql.ENUM('draft', 'reviewing', 'published', name='chapterstatus'),
        existing_nullable=True,
        postgresql_using='status::text'
    )
...
def downgrade() -> None:
    """恢复枚举类型，移除新增列。"""
    # 重建枚举类型
    novelstatus = postgresql.ENUM('planning', 'writing', 'completed', 'published', name='novelstatus', create_type=False)
    novelstatus.create(op.get_bind(), checkfirst=True)

    novellengthtype = postgresql.ENUM('short', 'medium', 'long', name='novellengthtype', create_type=False)
    novellengthtype.create(op.get_bind(), checkfirst=True)

    tasktype = postgresql.ENUM('planning', 'writing', 'editing', 'batch_writing', name='tasktype', create_type=False)
    tasktype.create(op.get_bind(), checkfirst=True)

    taskstatus = postgresql.ENUM('pending', 'running', 'completed', 'failed', 'cancelled', name='taskstatus', create_type=False)
    taskstatus.create(op.get_bind(), checkfirst=True)

    roletype = postgresql.ENUM('protagonist', 'supporting', 'antagonist', 'minor', name='roletype', create_type=False)
    ...
    characterstatus = postgresql.ENUM('alive', 'dead', 'unknown', name='characterstatus', create_type=False)
    characterstatus.create(op.get_bind(), checkfirst=True)

    chapterstatus = postgresql.ENUM('draft', 'reviewing', 'published', name='chapterstatus', create_type=False)
    chapterstatus.create(op.get_bind(), checkfirst=True)

    # 恢复列类型
    op.alter_column('chapters', 'status', type_=chapterstatus, postgresql_using='status::chapterstatus')
    op.alter_column('characters', 'status', type_=characterstatus, postgresql_using='status::characterstatus')
    op.alter_column('characters', 'gender', type_=gender_enum, postgresql_using='gender::gender')
    op.alter_column('characters', 'role_type', type_=roletype, postgresql_using='role_type::roletype')
    op.alter_column('generation_tasks', 'status', type_=taskstatus, postgresql_using='status::taskstatus')
    op.alter_column('generation_tasks', 'task_type', type_=tasktype, postgresql_using='task_type::tasktype')
    op.alter_column('novels', 'tags', type_=postgresql.ARRAY(sa.String()), postgresql_using="ARRAY(SELECT jsonb_array_elements_text(tags))")
    op.alter_column('novels', 'length_type', type_=novellengthtype, postgresql_using='length_type::novellengthtype')
    op.alter_column('novels', 'status', type_=novelstatus, postgresql_using='status::novelstatus')
