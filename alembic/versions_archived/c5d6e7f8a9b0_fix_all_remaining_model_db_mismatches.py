...
def upgrade() -> None:
    ...
    op.alter_column(
        ...
        existing_type=postgresql.ENUM(
            "active", "inactive", "expired", "error", name="accountstatus"
        ),
    ...
    op.create_index(
        "idx_agent_activities_novel_task", "agent_activities", ["novel_id", "task_id"]
    )
    ...
    op.create_index(
        "ix_agent_activities_agent_name", "agent_activities", ["agent_name"]
    )
    ...
    op.create_index(
        "ix_agent_activities_activity_type", "agent_activities", ["activity_type"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "changed_by", sa.String(100), nullable=False, server_default="system"
        ),
...
def downgrade() -> None:
    ...
    op.alter_column(
        ...
        "status",
        type_=accountstatus,
        postgresql_using="status::accountstatus",
    )

    publishtype = postgresql.ENUM(
        "create_book",
        "publish_chapter",
        "batch_publish",
        name="publishtype",
    ...
    op.alter_column(
        ...
        "publish_type",
        type_=publishtype,
        postgresql_using="publish_type::publishtype",
    )

    publishtaskstatus = postgresql.ENUM(
        "pending",
        "running",
        "completed",
        "failed",
    ...
    op.alter_column(
        "publish_tasks",
        "status",
        type_=publishtaskstatus,
        postgresql_using="status::publishtaskstatus",
    )

    publishstatus = postgresql.ENUM(
        "pending",
        "publishing",
        "published",
        "failed",
    ...
    op.alter_column(
        ...
        "status",
        type_=publishstatus,
        postgresql_using="status::publishstatus",
    )
