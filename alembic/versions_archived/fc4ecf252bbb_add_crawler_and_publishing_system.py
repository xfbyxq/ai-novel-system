...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('platform', sa.String(length=50), nullable=False, server_default='qidian'),
        sa.Column('crawl_type', sa.Enum('ranking', 'trending_tags', 'book_metadata', 'genre_list', name='crawltype', create_type=True), nullable=False),
        sa.Column('config', postgresql.JSONB(astext_type=sa.Text()), nullable=True),
        sa.Column('status', sa.Enum('pending', 'running', 'completed', 'failed', 'cancelled', name='crawltaskstatus', create_type=True), nullable=True),
    ...
    op.create_index('ix_crawler_tasks_platform_status', 'crawler_tasks', ['platform', 'status'])
    ...
    op.create_table(
        ...
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
        sa.ForeignKeyConstraint(['crawler_task_id'], ['crawler_tasks.id'], ondelete='CASCADE'),
        sa.PrimaryKeyConstraint('id')
    )
    ...
    op.create_table(
        ...
        sa.Column('status', sa.Enum('active', 'inactive', 'expired', 'error', name='accountstatus'), nullable=True),
    ...
    op.create_index('ix_platform_accounts_platform', 'platform_accounts', ['platform_name'])
    ...
    op.create_table(
        ...
        sa.Column('publish_type', sa.Enum('create_book', 'publish_chapter', 'batch_publish', name='publishtype'), nullable=False),
        sa.Column('target_chapters', postgresql.ARRAY(sa.Integer()), nullable=True),
        sa.Column('status', sa.Enum('pending', 'running', 'completed', 'failed', 'cancelled', name='publishtaskstatus'), nullable=True),
        sa.Column('progress', postgresql.JSONB(astext_type=sa.Text()), nullable=True),
        sa.Column('result_summary', postgresql.JSONB(astext_type=sa.Text()), nullable=True),
        sa.Column('error_message', sa.Text(), nullable=True),
        sa.Column('started_at', sa.DateTime(timezone=True), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column('status', sa.Enum('pending', 'publishing', 'published', 'failed', name='publishstatus'), nullable=True),
        sa.Column('error_message', sa.Text(), nullable=True),
    ...
    op.create_index('ix_chapter_publishes_task_id', 'chapter_publishes', ['publish_task_id'])

    # 扩展 reader_preferences 表
    op.add_column('reader_preferences', sa.Column('crawler_task_id', sa.UUID(), nullable=True))
    op.add_column('reader_preferences', sa.Column('book_id', sa.String(length=100), nullable=True))
    op.add_column('reader_preferences', sa.Column('book_title', sa.String(length=200), nullable=True))
    op.add_column('reader_preferences', sa.Column('author_name', sa.String(length=100), nullable=True))
    op.add_column('reader_preferences', sa.Column('rating', sa.Float(), nullable=True))
    op.add_column('reader_preferences', sa.Column('word_count', sa.Integer(), nullable=True))
...
def downgrade() -> None:
    ...
    op.drop_constraint('fk_reader_preferences_crawler_task', 'reader_preferences', type_='foreignkey')
