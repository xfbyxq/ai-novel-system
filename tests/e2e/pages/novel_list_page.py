...
class NovelListPage(BasePage):
    ...
    def fill_create_form(self, 
        ...
        # 选择匹配的选项
        option = self.page.locator(f".ant-select-dropdown .ant-select-item:has-text('{genre}')")
        if option.count() > 0:
            option.first.click()
        else:
            # 如果找不到精确匹配，选择第一个选项
        ...
        # 选择篇幅类型
        if length_type:
            length_select = self.get_element(self.SELECTORS["length_type_select"]).nth(0)
            length_select.click()
            ...
            option = self.page.locator(f".ant-select-dropdown .ant-select-item:has-text('{option_text}')")
            if option.count() > 0:
                option.first.click()
            else:
                self.page.locator(".ant-select-dropdown .ant-select-item").first.click()
