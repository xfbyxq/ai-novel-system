...
class NovelDetailPage(BasePage):
    ...
    def fill_chapter_generation_form(self, chapter_number: int):
        ...
        Args:
            chapter_number: 章节数
        """
        self.fill_input(self.OVERVIEW_SELECTORS["chapter_number_input"], str(chapter_number))
    ...
    def fill_batch_generation_form(self, start_chapter: int, end_chapter: int):
        ...
        Args:
            ...
            end_chapter: 结束章节数
        """
        self.fill_input(self.OVERVIEW_SELECTORS["batch_start_input"], str(start_chapter))
    ...
    def fill_edit_novel_form(self, title: str, genre: str, tags: list = None, synopsis: str = None):
        """
        填写编辑小说表单
    ...
    def get_chapter_titles(self) -> list:
        ...
        self.switch_to_chapters()
        titles = []
        title_elements = self.get_element(self.CHAPTERS_SELECTORS["chapter_title"]).all()
    ...
    def delete_chapter(self, chapter_index: int = 0):
        ...
        self.switch_to_chapters()
        # 点击删除按钮
        delete_buttons = self.get_element(self.CHAPTERS_SELECTORS["delete_chapter_btn"]).all()
        if chapter_index < len(delete_buttons):
            delete_buttons[chapter_index].click()
            # 确认删除
            self.wait_for_element_visible(self.CHAPTERS_SELECTORS["delete_confirm_modal"])
            self.click_element(self.CHAPTERS_SELECTORS["confirm_delete_btn"])
            self.wait_for_element_hidden(self.CHAPTERS_SELECTORS["delete_confirm_modal"])
    ...
    def wait_for_generation_complete(self, timeout: int = 60000):
        ...
        # 等待运行中的任务消失
        try:
            self.page.wait_for_selector("[data-testid*='running-task']", 
