...
NEO4J[Neo4j 图数据库<br/>端口: 7687, 7474<br/>仅内部访问]
end
subgraph "生产环境优化"
SECURITY[安全头配置]
GZIP[Gzip 压缩]
...
**图表来源**
- [docker-compose.prod.yml:34-59](file://docker-compose.prod.yml#L34-L59)
- [docker-compose.dev.yml:36-59](file://docker-compose.dev.yml#L36-L59)
- [nginx/nginx.conf:1-79](file://nginx/nginx.conf#L1-L79)

**章节来源**
- [docker-compose.prod.yml:1-125](file://docker-compose.prod.yml#L1-L125)
...
**更新** Neo4j 作为图数据库服务，提供复杂关系网络的存储和查询能力。**更新** 采用官方最新镜像并优化了安全配置。
...
系统采用微服务架构，通过 Docker 容器实现服务间的解耦和独立部署。新增的生产环境架构包含 Nginx 反向代理层和优化的安全配置。
...
**更新** 生产环境使用 docker-compose.prod.yml 进行服务编排，包含完整的 Nginx 反向代理、优化的容器网络设置和安全的 Neo4j 配置：
...
BACKEND --> GRAPH_DB[Neo4j 图数据库<br/>仅内部访问]
end
```
...
NEO4J[Neo4j<br/>容器名: neo4j<br/>仅内部访问]
...
- **容器别名**：通过服务名进行内部通信
- **安全隔离**：Neo4j 仅内部访问，不暴露外部端口
- **健康检查**：自动监控容器状态

#### 端口配置

| 服务 | 容器端口 | 宿主机端口 | 用途 | 安全性 |
|------|----------|------------|------|--------|
| PostgreSQL | 5432 | 无映射 | 内部数据库服务 | 仅内部访问 |
| Redis | 6379 | 无映射 | 内部缓存服务 | 仅内部访问 |
| Neo4j | 7687 | 无映射 | Bolt 协议 | 仅内部访问 |
| Neo4j | 7474 | 无映射 | HTTP 管理界面 | 仅内部访问 |
| Backend | 8000 | 8080 | API 服务 | 通过 Nginx 代理 |
| Nginx | 80 | 80 | 反向代理 | 对外暴露 |

**更新** 生产环境中 Neo4j 的端口映射已被完全移除，仅允许容器内部访问，显著提高了安全性。

**章节来源**
- [docker-compose.prod.yml:115-125](file://docker-compose.prod.yml#L115-L125)
...
NEO4J_DEV --> PORT_MAPPING_DEV[端口映射开发]
PORT_MAPPING_DEV --> READY[开发环境就绪]
```
...
**更新** 开发环境保留了 Neo4j 的端口映射，便于开发和调试。
...
**更新** 系统集成了 Neo4j 图数据库，提供强大的图数据分析能力。**更新** 采用官方最新镜像并优化了安全配置。
...
**更新** 生产环境中的 Neo4j 仅通过内部网络访问，不暴露外部端口，提高了安全性。

**章节来源**
- [docker-compose.prod.yml:34-59](file://docker-compose.prod.yml#L34-L59)
- [docker-compose.dev.yml:36-59](file://docker-compose.dev.yml#L36-L59)
- [backend/config.py:315-341](file://backend/config.py#L315-L341)
...
# Neo4j 管理界面：不可直接访问（仅内部）
```

**更新** 生产环境中的 Neo4j 管理界面不再对外暴露，仅允许容器内部访问。
...
docker-compose -f docker-compose.prod.yml port backend 8080
...
**更新** 移除 PYTHONPATH 后的依赖管理问题和解决方案
...
**更新** Neo4j 图数据库特有的故障排除指南，包括安全配置问题
...
# 通过 Docker exec 访问 Neo4j
docker-compose -f docker-compose.prod.yml exec neo4j cypher-shell -u neo4j -p ${NEO4J_PASSWORD:-novel_graph_pass}
...
小说生成系统的 Docker 部署方案经过重大升级，提供了更加完善的企业级部署解决方案。通过新增的生产环境配置、Nginx 反向代理和优化的 Neo4j 图数据库实例，系统具备了以下优势：
...
5. **智能实体抽取**：从章节内容中自动识别实体
6. **一致性冲突检测**：检测故事逻辑冲突
7. **影响力分析**：计算角色影响力评分
8. **强大的自动化能力**：从构建到部署的全流程自动化
9. **灵活的环境配置**：支持开发、测试和生产环境的无缝切换
10. **生产环境安全优化**：移除外部端口暴露，显著提高安全性

**更新** 最新的生产环境配置 docker-compose.prod.yml 提供了企业级的部署能力，包含 Nginx 反向代理、优化的容器网络设置和安全的 Neo4j 配置。**更新** Neo4j 采用官方最新镜像 `neo4j:latest`，并移除了外部端口暴露，仅允许容器内部访问，显著提高了系统的安全性。简化后的开发环境配置专注于开发效率和调试便利性。移除 PYTHONPATH 环境变量后，依赖管理流程更加简洁高效，显著提高了容器化部署的整体效率。
...
该部署方案为小说生成系统的稳定运行和持续发展奠定了坚实的基础，为后续的功能扩展和技术演进提供了良好的基础设施支持。**更新** 生产环境配置特别适合企业级部署，提供了更好的性能、安全性和可维护性。**更新** 简化的开发环境配置提升了开发效率，为开发者提供了更好的开发体验。**更新** 移除 PYTHONPATH 环境变量后，整体部署流程更加简洁高效，为系统的长期维护和发展提供了更好的基础。**更新** Neo4j 图数据库的集成使得系统具备了强大的关系分析能力，**更新** 官方镜像和安全配置优化为小说创作提供了更深入的洞察和分析支持。