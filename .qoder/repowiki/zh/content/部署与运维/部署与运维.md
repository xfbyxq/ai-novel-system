...
## 更新摘要
**所做变更**
- 更新了项目版本信息：从1.2.0升级到2.0.0
- 新增了Poetry依赖管理的详细说明，包括Selenium、trio、WebSocket等新增依赖
- 新增了Healenium服务集成的完整说明，包括Docker部署配置和测试框架集成
- 更新了容器化部署流程，增加了多阶段构建和依赖管理优化
- 新增了智能测试框架的部署和配置指南
...
**更新** 本版本特别强调了Poetry依赖管理和Healenium智能测试框架的集成，项目当前版本为2.0.0，包含Selenium自动化测试、trio异步编程支持、WebSocket实时通信等重要功能改进。

## 项目结构
该系统采用前后端分离与后端服务化架构：后端基于FastAPI提供REST API；数据库使用PostgreSQL；缓存使用Redis；任务队列使用Celery + Redis；智能体系统通过AgentManager集中管理；前端使用Vite+React；测试包含Playwright端到端测试和Healenium智能自愈测试；数据库迁移使用Alembic。
...
HT["Healenium服务<br/>智能测试框架"]
end
subgraph "基础设施"
PG["数据库<br/>PostgreSQL"]
RD["缓存/消息<br/>Redis"]
CW["任务队列<br/>Celery"]
SE["Selenium Grid<br/>UI自动化"]
TR["Trio异步<br/>并发处理"]
...
## 核心组件
- 应用入口与路由：后端以FastAPI为主入口，注册CORS中间件与API路由，提供根与健康检查端点，版本信息为2.0.0。
...
- **新增** Healenium智能测试框架：提供元素定位自愈、选择器评分、AI辅助修复等功能。
- **新增** Poetry依赖管理系统：支持开发依赖、生产依赖分离，提供锁定文件一致性保证。
...
## 架构总览
系统采用微服务风格的单体后端，辅以外部数据库、缓存与任务队列。容器编排通过docker-compose快速拉起PostgreSQL与Redis；后端服务暴露REST API；智能体系统与任务队列解耦；前端通过API与后端交互。**新增** Healenium服务作为独立组件集成，提供智能测试自愈能力。
...
- **新增** Healenium服务集成：在测试环境中提供智能选择器修复能力。
...
- **新增** Selenium 4.27.0：提供强大的Web自动化测试能力。
- **新增** trio 0.26.2：支持现代异步编程模式。
- **新增** websockets 14.0：提供WebSocket协议支持。
- **新增** playwright 1.58.0：现代化的浏览器自动化测试框架。
- 依赖锁定文件确保环境一致性，支持多平台兼容性。

```mermaid
flowchart TD
Start(["Poetry依赖管理"]) --> Install["安装依赖<br/>poetry install"]
Install --> Lock["锁定依赖<br/>poetry lock"]
Lock --> Sync["同步环境<br/>poetry env use"]
Sync --> Build["构建镜像<br/>多阶段构建"]
Build --> Deploy["部署应用<br/>容器化"]
```
...
### Healenium智能测试框架集成
- **新增** Healenium服务：提供元素定位自愈能力，支持智能选择器修复。
- **新增** HealeniumAdapter：封装与Healenium Server的通信，支持健康检查与超时处理。
- **新增** SelectorManager：统一管理选择器历史、回退策略和评分机制。
- **新增** 多种回退策略：CSS到文本、Ant Design组件、XPath转换、部分匹配等。
- **新增** AI辅助修复：结合大语言模型提供智能选择器建议。
...
HealeniumAdapter --> HealeniumConfig : "使用"
SelectorManager --> HealeniumAdapter : "依赖"
```

**图表来源**
- [tests/ai_e2e/selectors/healenium_adapter.py:39-397](file://tests/ai_e2e/selectors/healenium_adapter.py#L39-L397)
- [tests/ai_e2e/config.py:32-39](file://tests/ai_e2e/config.py#L32-L39)

**章节来源**
- [tests/ai_e2e/selectors/healenium_adapter.py:1-397](file://tests/ai_e2e/selectors/healenium_adapter.py#L1-L397)
- [tests/ai_e2e/config.py:1-92](file://tests/ai_e2e/config.py#L1-L92)
...
- **新增** 主要版本改进：Poetry依赖管理、Healenium智能测试框架、Selenium自动化测试
- 变更日志：详细记录每个版本的功能改进、Bug修复和安全更新
- 发布流程：基于Git标签的自动化发布，包含版本号更新与文档同步

**更新** 项目已升级到2.0.0版本，包含Poetry依赖管理、Healenium智能测试框架、Selenium自动化测试等重要改进。
...
## 依赖关系分析
系统主要依赖关系如下：后端依赖数据库与缓存；任务队列依赖缓存；智能体系统依赖缓存与任务队列；日志系统贯穿各模块；配置中心为所有组件提供统一参数；**新增** Healenium服务依赖Selenium Grid和Trio异步框架。
...
- **新增** Healenium性能优化：合理设置评分阈值与超时时间，避免影响测试执行效率。
- **新增** Poetry依赖优化：使用锁定文件确保依赖版本一致性，减少运行时冲突。
...
- 版本兼容性问题
  - 确认所有组件版本兼容性，特别是FastAPI、SQLAlchemy、Celery等核心依赖。
  - 检查数据库迁移是否与当前版本匹配。
- **新增** Healenium服务问题
  - 检查Healenium服务端点可达性；验证评分阈值设置。
  - 查看选择器修复日志与超时配置。
- **新增** Poetry依赖问题
  - 清理虚拟环境并重新安装依赖；检查poetry.lock文件一致性。
  - 验证Python版本与依赖兼容性。

**更新** 新增了Healenium服务和Poetry依赖相关的故障排除指导。
...
## 结论
本指南提供了小说生成系统的端到端部署与运维实践，涵盖容器化、配置管理、任务调度、智能体系统、数据库迁移、CI/CD与监控等关键环节。**更新** 本版本特别强调了Poetry依赖管理和Healenium智能测试框架的集成，项目当前版本为2.0.0，包含Selenium自动化测试、trio异步编程支持、WebSocket实时通信、智能测试自愈等重要功能改进。建议在生产环境中进一步完善安全加固、负载均衡与SSL、备份恢复与滚动更新策略，并结合容量规划与性能压测，确保系统稳定与可扩展。
...
- 自动化测试
  ...
  - **新增** Selenium自动化测试与Healenium智能测试。
- 持续集成与部署
...
- 性能指标
  ...
  - **新增** Healenium服务监控：请求成功率、平均修复时间、评分分布。
- 日志管理
  - 统一日志采集与归档，建立索引与检索能力。
- 告警机制
...
### Healenium智能测试框架部署（建议）
- 服务部署
  - 在测试环境中部署Healenium Server，提供智能选择器修复能力
  - 配置合适的评分阈值和超时时间
- 集成测试
  - 将Healenium集成到现有测试框架中
  - 实现选择器历史记录和学习机制
- 性能优化
  - 监控Healenium服务性能指标
  - 优化选择器修复算法和评分机制
