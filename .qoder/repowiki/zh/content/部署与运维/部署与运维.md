...
- [create_tables.sh](file://create_tables.sh)
...
## 更新摘要
**所做变更**
- 新增Docker配置文件章节，详细介绍docker-compose.dev.yml和docker-compose.migration.yml的使用方法
- 新增数据库迁移脚本章节，涵盖migrate_db.sh、run_migration.sh、redeploy_with_migration.sh等运维工具
- 更新容器化部署能力说明，包括开发环境、生产环境和迁移环境的差异化配置
- 新增数据库修复和初始化脚本，提供完整的数据库管理解决方案
- 增强开发环境启动脚本，提供代码热更新和本地开发模式支持
...
**更新** 本版本特别增强了Docker容器化部署能力，新增了专门的开发环境配置、迁移环境配置、数据库迁移脚本和修复工具，提供更完善的容器化解决方案和数据库管理能力。
...
- 配置中心：统一读取环境变量，动态拼接数据库、Redis、Celery等连接串，支持Docker环境自动检测。
...
### Docker容器化与部署脚本
- **deploy_docker.sh**：提供一键部署功能，包含停止旧容器、构建镜像、启动服务和状态检查的完整流程
- **rebuild_docker.sh**：提供完全重建功能，包含清理旧容器、删除悬空镜像、无缓存重建和详细日志查看
- **docker-start.sh**：传统启动脚本，支持构建和启动服务，包含健康检查和日志查看
- **docker-stop.sh**：优雅停止所有服务，支持可选的Docker资源清理

**更新** 新增了专门的部署脚本，简化了容器化部署流程，提供了更完善的自动化部署能力。
...
### Docker配置文件详解
- **docker-compose.yml**：生产环境配置，包含健康检查、重启策略、环境变量等
- **docker-compose.dev.yml**：开发环境配置，使用代码挂载实现热更新，支持调试模式
- **docker-compose.migration.yml**：迁移环境配置，专用于数据库迁移操作
- **环境变量自动检测**：通过DOCKER_ENV变量自动切换数据库和Redis连接地址
- **开发模式优化**：开发环境启用代码热更新和调试功能

**更新** 新增了专门的开发环境配置和迁移环境配置，提供了更完善的容器化部署能力。
...
### Docker配置优化
- **Dockerfile.backend**：优化了国内镜像源配置，使用阿里云pip源和apt镜像，提升依赖安装速度
- **Dockerfile.frontend**：前端镜像同样配置了国内npm镜像源，确保开发环境的网络稳定性
- **多阶段构建**：后端使用Python slim镜像，前端使用Node Alpine镜像，减小镜像体积
- **重试机制**：Poetry安装增加了重试逻辑，提高构建稳定性

**更新** Dockerfile.backend经过优化，增加了国内镜像源支持和安装重试机制，显著提升了构建效率。
...
### 健康检查与环境变量配置
- **容器健康检查**：PostgreSQL和Redis配置了健康检查，确保服务可用性
- **后端健康检查**：通过/health端点检查数据库和Redis连接状态
- **环境变量管理**：统一的Settings类管理所有配置，支持开发和生产环境切换
- **Docker环境检测**：自动识别Docker环境并调整连接参数

**更新** 健康检查配置更加完善，包含了容器层面和应用层面的双重健康检查机制。

**章节来源**
- [docker-compose.yml:13-77](file://docker-compose.yml#L13-L77)
...
### 数据库迁移管理
- **migrate_db.sh**：使用asyncpg驱动的Python迁移脚本，支持在容器内执行数据库迁移
- **run_migration.sh**：直接在运行容器中执行SQL迁移，提供Alembic和手动SQL两种迁移方式
- **redeploy_with_migration.sh**：完整的重新部署脚本，包含数据库迁移的完整流程
- **create_tables.sh**：直接在PostgreSQL容器中创建所有表结构
- **fix_database.sh**：修复数据库表缺失问题，支持Alembic和SQLAlchemy两种修复方式
- **fix_database_types.sh**：修复数据库缺失的枚举类型和字段
...
- [create_tables.sh:1-310](file://create_tables.sh#L1-L310)
- [fix_database.sh:1-144](file://fix_database.sh#L1-L144)
- [fix_database_types.sh:1-137](file://fix_database_types.sh#L1-L137)

### 开发环境启动脚本
- **start_dev.sh**：开发环境快速启动脚本，支持代码热更新和数据库初始化
- **start_local_dev.sh**：本地开发模式启动脚本，不使用Docker打包，适合原生开发环境
- **开发环境特性**：支持代码热更新、调试模式、数据库自动初始化
- **一键启动**：提供完整的开发环境启动流程，包含服务检查和日志查看

**新增章节**

**章节来源**
- [start_dev.sh:1-102](file://start_dev.sh#L1-L102)
...
- Docker优化：使用国内镜像源提升构建速度，合理的镜像分层减少构建时间。
- **数据库迁移优化**：使用专用迁移容器，避免与业务容器竞争资源；支持并行迁移和回滚机制。

**更新** Docker配置优化显著提升了构建性能，国内镜像源的使用大幅减少了依赖下载时间。新增数据库迁移优化，提供专用迁移容器和并行处理能力。
...
- Docker部署问题
  - 使用deploy_docker.sh进行标准部署流程
  - 使用rebuild_docker.sh进行完全重建
  - 检查容器健康状态和日志输出
- **数据库迁移问题**
  - 使用run_migration.sh进行迁移操作，支持回退到手动SQL迁移
  - 使用fix_database.sh修复缺失的表结构
  - 使用fix_database_types.sh修复缺失的枚举类型和字段
- **开发环境问题**
  - 使用start_dev.sh快速启动开发环境
  - 使用start_local_dev.sh进行本地开发模式
  - 检查代码挂载和热更新配置

**更新** 新增了数据库迁移相关的故障排除指导，包括迁移脚本使用、数据库修复和开发环境启动问题。
...
## 结论
本指南提供了小说生成系统的端到端部署与运维实践，涵盖容器化、配置管理、任务调度、智能体系统、数据库迁移、CI/CD与监控等关键环节。**更新** 本版本特别增强了Docker容器化部署能力，新增了专门的部署脚本、优化的Docker配置、完善的开发/生产环境差异化配置和数据库管理工具。建议在生产环境中进一步完善安全加固、负载均衡与SSL、备份恢复与滚动更新策略，并结合容量规划与性能压测，确保系统稳定与可扩展。
...
- **镜像构建优化**
  - 使用国内镜像源提升依赖安装速度
  - Dockerfile.backend和Dockerfile.frontend都配置了相应的镜像源
  - 增加了安装重试机制，提高构建稳定性
- **容器编排配置**
  - docker-compose.yml提供生产环境配置，包含健康检查和重启策略
  - docker-compose.dev.yml提供开发环境配置，支持代码热更新
  - docker-compose.migration.yml提供迁移环境配置，专用于数据库迁移
  - 支持Docker环境自动检测和环境变量切换
- **部署脚本使用**
  - deploy_docker.sh：一键部署，适合常规更新
  - rebuild_docker.sh：完全重建，适合环境清理和重大变更
  - docker-start.sh/docker-stop.sh：传统部署方式
...
### 数据库迁移与管理工具
- **迁移脚本**
  - migrate_db.sh：使用asyncpg驱动的Python迁移脚本
  - run_migration.sh：直接在容器中执行SQL迁移，支持回退机制
  - redeploy_with_migration.sh：完整的重新部署脚本，包含迁移流程
- **初始化脚本**
  - create_tables.sh：直接在PostgreSQL容器中创建所有表结构
  - fix_database.sh：修复缺失的表结构，支持多种修复方式
  - fix_database_types.sh：修复缺失的枚举类型和字段
- **开发环境脚本**
  - start_dev.sh：开发环境快速启动，支持热更新
  - start_local_dev.sh：本地开发模式启动
...
### Docker部署最佳实践
- **标准化部署流程**
  - 使用deploy_docker.sh进行常规部署，确保一致的部署流程
  - 使用rebuild_docker.sh进行完全重建，适用于环境污染或重大变更
  - 遵循"停止-构建-启动-检查"的标准流程
- **环境隔离**
  - 开发环境使用docker-compose.dev.yml，支持热更新和调试
  - 生产环境使用docker-compose.yml，包含健康检查和重启策略
  - 迁移环境使用docker-compose.migration.yml，专用于数据库迁移
  - 通过DOCKER_ENV变量自动切换环境配置
- **监控与维护**
  - 定期检查容器健康状态和日志输出
  - 使用docker-compose ps和docker-compose logs进行状态监控
  - 建立容器资源使用监控和清理机制
- **数据库管理**
  - 使用专用迁移脚本和工具链
  - 支持回滚和修复机制
