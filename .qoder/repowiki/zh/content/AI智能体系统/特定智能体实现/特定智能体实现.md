...
- [tests/agents/test_continuity_components.py](file://tests/agents/test_continuity_components.py)
...
- 新增连贯性集成模块（ContinuityIntegrationModule）整合多个连贯性组件
...
**更新** 系统现已集成多项新增智能体，包括质量评估、主题守护、连贯性检查和投票共识等，形成更加完善的智能体生态系统。特别是新增的预防式连贯性检查智能体，提供了强大的连贯性保障能力。
...
CIM["continuity_integration_module.py<br/>连贯性集成模块"]
...
- [agents/prevention_continuity_checker.py:1-712](file://agents/prevention_continuity_checker.py#L1-L712)
- [agents/continuity_integration_module.py:1-655](file://agents/continuity_integration_module.py#L1-L655)
...
**更新** 新增质量评估、主题守护、连贯性检查和投票管理等智能体，形成完整的质量保障体系。特别是预防式连贯性检查智能体，提供了强大的连贯性保障能力。
...
- 职责边界：整合多个连贯性保障组件，提供统一的接口来管理所有连贯性保障组件。
- 输入：小说ID、小说数据、章节号、章节摘要、章节内容、冲突列表。
- 输出：包含主题审查、大纲验证、角色一致性、伏笔检查、预防检查等结果的综合报告。
- 决策逻辑：协调各个连贯性组件的工作，计算综合评分，汇总问题和建议。
- 错误处理：各组件失败时记录警告，不影响整体流程。

```mermaid
flowchart TD
Start(["开始连贯性集成"]) --> InitComponents["初始化各组件<br/>上下文管理器、主题守护者、大纲映射器、角色追踪器、伏笔注入器、预防检查器"]
InitComponents --> PrepareContext["准备章节生成上下文<br/>构建增强上下文、大纲任务、主题指导、伏笔要求"]
PrepareContext --> ReviewPlan["审查章节策划<br/>主题一致性、大纲验证、角色一致性、伏笔检查、预防检查"]
ReviewPlan --> NewChecks["新增检查步骤<br/>世界观演变、空间一致性、角色关系、伏笔自动检测、统一评分卡"]
NewChecks --> AggregateResults["汇总结果<br/>综合评分、问题汇总、建议生成"]
AggregateResults --> End(["结束"])
```

**图表来源**
- [agents/continuity_integration_module.py:116-185](file://agents/continuity_integration_module.py#L116-L185)
- [agents/continuity_integration_module.py:326-515](file://agents/continuity_integration_module.py#L326-L515)
...
- 组件耦合
  ...
  - **新增智能体**：quality_evaluator依赖qwen_client和cost_tracker；theme_guardian处理主题定义和审查；prevention_continuity_checker处理连贯性检查；continuity_integration_module整合多个连贯性组件；voting_manager协调多智能体投票。
- 外部依赖
...
CIM["continuity_integration_module.py"] --> QC
CIM --> CT
...
- **新增连贯性检查优化**
  - PreventionContinuityChecker提供详细的评分权重（约束回应40%、情节一致性35%、剧情推进25%），确保全面评估。
  - ContinuityIntegrationModule整合多个组件，提供统一的连贯性保障接口。
...
- [agents/prevention_continuity_checker.py:118-126](file://agents/prevention_continuity_checker.py#L118-L126)
- [agents/continuity_integration_module.py:490-506](file://agents/continuity_integration_module.py#L490-L506)
...
- 常见问题
  ...
  - **新增智能体问题**：
    ...
    - ContinuityIntegrationModule集成失败：检查各组件初始化和依赖关系。
...
- [agents/continuity_integration_module.py:624-635](file://agents/continuity_integration_module.py#L624-L635)
...
**更新** 新增的质量评估、主题守护、连贯性检查和投票管理等智能体，形成了更加完善的质量保障体系。质量评估支持五维评分和加权计算，主题守护确保内容主题一致性，预防式连贯性检查在生成前发现问题，连贯性集成模块整合多个组件提供统一接口，投票管理器协调多智能体决策。这些新增功能显著提升了系统的智能化水平和内容质量保证能力。

特别是预防式连贯性检查智能体，提供了强大的连贯性保障能力，通过约束回应检查、情节一致性分析和剧情推进评估，确保章节策划的质量。连贯性集成模块进一步增强了系统的连贯性保障能力，整合了上下文管理、主题守护、大纲映射、角色追踪、伏笔注入等多个组件。
...
- 配置参数建议
  ...
  - **新增智能体配置**：
    ...
    - PreventionContinuityChecker：约束权重0.4，一致性权重0.35，进度权重0.25，通过阈值7.0。
    - ContinuityIntegrationModule：综合评分阈值7.0，支持多组件集成。
...
- 最佳实践
  ...
  - **新增智能体使用建议**：
    ...
    - 连贯性集成：使用ContinuityIntegrationModule统一管理多个连贯性组件。
