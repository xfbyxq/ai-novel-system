...
章节来源
- [llm/qwen_client.py:1-269](file://llm/qwen_client.py#L1-L269)
...
**更新** 重点更新了流式输出的实现细节，强调了await异步流对象的修复
...
- [llm/__init__.py:1-6](file://llm/__init__.py#L1-L6)
...
## 结论
QwenClient提供了统一、健壮的通义千问调用封装，支持OpenAI兼容模式与标准SDK两种路径，具备完善的异步与重试机制、流式输出能力以及成本追踪支持。**本次更新修复了流式响应处理中的关键缺陷**，通过正确await异步流对象确保了可靠的实时响应处理和防止竞态条件。结合编排层与服务层的协同，可高效支撑小说创作全流程的自动化生成与发布，同时支持实时流式对话功能。
