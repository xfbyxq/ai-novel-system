...
**更新** 新增AiChatService作为流式对话服务层的使用示例
...
- [backend/config.py:5-425](file://backend/config.py#L5-L425)

章节来源
- [llm/qwen_client.py:1-350](file://llm/qwen_client.py#L1-L350)
- [backend/config.py:1-425](file://backend/config.py#L1-L425)

## 核心组件
- QwenClient：统一的通义千问客户端封装，支持OpenAI兼容模式与标准DashScope SDK；提供同步/异步聊天与流式输出；内置指数退避重试；支持工具调用功能。
...
**更新** 重点更新了流式输出的实现细节，强调了await异步流对象的修复和新增工具调用功能
...
- [llm/qwen_client.py:194-350](file://llm/qwen_client.py#L194-L350)
...
图表来源
- [llm/qwen_client.py:99-130](file://llm/qwen_client.py#L99-L130)
- [llm/qwen_client.py:147-192](file://llm/qwen_client.py#L147-L192)
...
图表来源
- [llm/qwen_client.py:208-213](file://llm/qwen_client.py#L208-L213)
- [llm/qwen_client.py:217-239](file://llm/qwen_client.py#L217-L239)
- [llm/qwen_client.py:254-264](file://llm/qwen_client.py#L254-L264)
...
章节来源
- [llm/qwen_client.py:248-345](file://llm/qwen_client.py#L248-L345)
...
- 服务层依赖
  ...
  - **新增** backend/services/ai_chat_service.py依赖QwenClient实现流式对话功能

**更新** 新增AiChatService的依赖关系
...
**更新** 强调了await异步流对象修复对性能和可靠性的影响和新增超时优化
...
**更新** 新增了流式响应竞态条件和工具调用的故障排查指南

章节来源
- [llm/qwen_client.py:117-130](file://llm/qwen_client.py#L117-L130)
- [llm/qwen_client.py:179-192](file://llm/qwen_client.py#L179-L192)
- [llm/qwen_client.py:237-239](file://llm/qwen_client.py#L237-L239)

## 结论
QwenClient提供了统一、健壮的通义千问调用封装，支持OpenAI兼容模式与标准SDK两种路径，具备完善的异步与重试机制、流式输出能力、工具调用功能以及成本追踪支持。**本次更新显著增强了客户端功能**，通过正确await异步流对象确保了可靠的实时响应处理和防止竞态条件，新增的工具调用功能支持复杂的函数调用场景，扩展的超时配置（最高300秒）为复杂任务提供了更好的稳定性。结合编排层与服务层的协同，可高效支撑小说创作全流程的自动化生成与发布，同时支持实时流式对话功能。

**更新** 强调了本次更新的核心价值和修复的重要性
