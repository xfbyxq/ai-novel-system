...
- [alembic/versions/3c70dad7710e_add_performance_indexes.py](file://alembic/versions/3c70dad7710e_add_performance_indexes.py)
- [alembic/versions_archived/117e653b6d9a_add_agent_name_column_to_token_usages_.py](file://alembic/versions_archived/117e653b6d9a_add_agent_name_column_to_token_usages_.py)
</cite>
...
- **新增** 预算控制功能设计，包括月度预算设置、实时余额监控、超支预警机制
- **新增** 限额告警系统，包括阈值配置、通知机制、自动暂停策略
...
**更新** 本版本新增了完整的CostTracker类和TokenUsage模型，实现了详细的token使用监控和成本追踪功能，包括章节级别成本分解和多代理成本统计。新增章节级成本追踪、成本类别分类（base/iteration/query/vote）、成本限制检查、SQLAlchemy模型集成等功能。同时新增预算控制和限额告警系统的设计建议，提供更精细的成本管理能力。
...
### 功能概述
预算控制功能提供月度预算设置、实时余额监控和超支预警机制，帮助用户在成本控制方面实现精细化管理。

### 核心特性
- **月度预算设置**：支持为不同章节、Agent类型设置独立预算
- **实时余额监控**：提供实时成本余额查询和预算使用率计算
- **超支预警机制**：支持多级预警阈值设置和自动通知
- **预算调整**：支持预算的动态调整和历史记录追踪

### 实现机制
```mermaid
flowchart TD
BudgetSetup["预算设置"] --> MonthlyBudget["月度预算配置<br/>按章节/Agent类型设置"]
MonthlyBudget --> RealTimeMonitor["实时余额监控<br/>成本累计与预算对比"]
RealTimeMonitor --> ThresholdCheck["阈值检查<br/>预算使用率阈值判断"]
ThresholdCheck --> AlertTrigger{"超过预警阈值?"}
AlertTrigger --> |是| SendAlert["发送预警通知<br/>邮件/短信/系统通知"]
AlertTrigger --> |否| Normal["正常状态"]
SendAlert --> AutoPause["自动暂停策略<br/>超阈值时暂停相关任务"]
Normal --> BudgetReport["生成预算报告<br/>使用率、剩余预算、趋势分析"]
AutoPause --> BudgetReport
```

### 预算配置接口
- `set_monthly_budget(budget_type: str, amount: float, period: str = "monthly")`：设置月度预算
- `get_budget_balance(budget_type: str) -> dict`：获取预算余额和使用情况
- `check_budget_threshold(budget_type: str, threshold: float) -> bool`：检查预算阈值
- `get_budget_report(period: str = "monthly") -> dict`：生成预算使用报告

### 预算类型分类
- **章节级预算**：按小说章节设置预算，支持base/iteration/query/vote四类成本
- **Agent级预算**：按Agent类型设置预算，支持不同Agent的成本控制
- **全局预算**：整体成本预算控制，防止总成本超支
...
### 功能概述
限额告警系统提供多层级的成本限制检查、通知机制和自动暂停策略，确保成本控制的有效执行。

### 核心特性
- **多级阈值配置**：支持预算使用率阈值（如80%、90%、100%）的灵活配置
- **通知机制**：支持邮件、短信、系统内通知等多种通知方式
- **自动暂停策略**：超限时自动暂停相关Agent任务执行
- **告警历史**：记录所有告警事件和处理状态

### 实现机制
```mermaid
flowchart TD
CostCalc["成本计算"] --> BudgetCheck["预算检查<br/>使用率计算"]
BudgetCheck --> Threshold{"超过阈值?"}
Threshold --> |80%| Warning["发送预警通知"]
Threshold --> |90%| Warning
Threshold --> |100%| Critical["严重超支告警"]
Critical --> AutoPause["自动暂停相关任务"]
Warning --> Monitor["继续监控"]
AutoPause --> Monitor
Monitor --> AlertHistory["记录告警历史"]
```

### 告警级别定义
- **一级告警（80%使用率）**：发送预警通知，提醒用户关注成本使用情况
- **二级告警（90%使用率）**：发送紧急通知，建议用户考虑成本控制措施
- **三级告警（100%使用率）**：发送严重告警，自动暂停相关任务执行

### 通知配置
- **通知渠道**：邮件、短信、企业微信、钉钉等
- **接收人员**：项目负责人、财务人员、系统管理员
- **通知模板**：标准化的告警通知模板，包含预算信息、使用情况、建议措施

### 自动暂停策略
- **暂停范围**：根据告警级别暂停相应范围的任务执行
- **恢复条件**：预算恢复到安全阈值或手动解除暂停
- **恢复流程**：自动恢复或人工确认恢复

**章节来源**
- [llm/cost_tracker.py:94-100](file://llm/cost_tracker.py#L94-L100)
- [backend/services/monitoring_service.py:275-369](file://backend/services/monitoring_service.py#L275-L369)
...
- 组件耦合
  ...
  - **新增** 预算控制模块与CostTracker的集成，提供预算管理和告警功能
- 外部依赖
...
BudgetControl["预算控制模块"] --> CostTracker
AlertSystem["限额告警系统"] --> CostTracker
```
...
- **新增** Agent成本统计的缓存机制
  ...
  - 提供成本统计的定期清理和过期机制
- **新增** 预算控制的性能优化
  - 预算数据采用内存缓存，减少数据库查询频率
  - 告警阈值检查采用批量处理，避免频繁的阈值计算
...
- **新增** 预算控制功能设计提供了月度预算设置、实时余额监控、超支预警机制的完整解决方案
- **新增** 限额告警系统提供了多级阈值配置、通知机制、自动暂停策略的告警管理能力
