...
## 更新摘要
**所做更改**
- 新增章节级别成本监控功能，支持按章节维度追踪Token使用和成本
- 增加多代理费用追踪能力，支持不同Agent类型的成本分类统计
- 扩展CostTracker以支持章节号和成本类别参数
- 更新Agent体系以利用新的章节和成本分类追踪功能
- 增强收益分析服务以支持章节级别的成本分析
...
**更新** 本版本新增章节级别成本监控和多代理费用追踪能力，提供更精细的成本控制和分析功能。
...
- [llm/cost_tracker.py:1-120](file://llm/cost_tracker.py#L1-L120)
...
## 核心组件
- CostTracker：LLM调用的Token与成本追踪器，提供单次记录、累计统计与摘要导出，现已支持章节级别和成本分类追踪
- QwenClient：DashScope/OpenAI兼容模式的异步调用封装，统一返回usage结构
- Agent体系：AgentManager集中管理Agent；SpecificAgents与CrewManager在任务执行中调用LLM并记录成本，支持章节和成本类别追踪
...
**章节来源**
- [llm/cost_tracker.py:16-120](file://llm/cost_tracker.py#L16-L120)
...
## 架构总览
下图展示了从Agent到LLM、成本追踪、数据库落库与API消费的全链路，包括新增的章节级别追踪功能。
...
UpdateStats --> AppendRecord["追加记录到records<br/>包含章节号和成本类别"]
AppendRecord --> ChapterTracking["按章节追踪成本<br/>支持base/iteration/query/vote分类"]
...
- [llm/cost_tracker.py:26-120](file://llm/cost_tracker.py#L26-L120)
- [llm/qwen_client.py:54-64](file://llm/qwen_client.py#L54-L64)
...
**章节来源**
- [llm/cost_tracker.py:84-120](file://llm/cost_tracker.py#L84-L120)
...
- 组件耦合
  ...
  - 服务层依赖数据库模型进行聚合统计
  - **新增** CostTracker与Agent组件的紧密耦合，支持章节和类别参数传递
- 外部依赖
...
QwenClient --> CostTracker["CostTracker<br/>章节级追踪"]
