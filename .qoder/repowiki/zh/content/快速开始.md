...
## 简介
本指南面向首次部署"小说生成系统"的用户，目标是在约30分钟内完成从环境准备、容器化部署、数据库初始化、智能体启动到前端访问的全流程，并体验核心功能。系统采用 Python 3.12+、FastAPI 后端、React 前端、PostgreSQL 数据库、Redis 缓存与 Celery 异步队列，配合 Docker Compose 进行一键部署。**版本升级**：当前版本为 1.0.0，这是初始生产发布版本，包含完整的功能集和生产就绪的安全配置。
...
- **版本升级**：1.0.0 版本包含完整的生产就绪配置，禁用开发调试模式

**章节来源**
- [backend/main.py](file://backend/main.py#L5-L53)
...
## 架构总览
系统采用"容器化基础设施 + 微服务后端 + 前端 SPA"的架构。容器提供数据库与缓存；后端提供 REST API；前端通过代理访问后端；Celery 通过 Redis 承载异步任务。**版本升级**：1.0.0 版本提供生产就绪的完整功能集，包含 Docker 完整容器化部署支持、内存服务增强、AI 聊天会话隔离等功能。
...
- [backend/main.py](file://backend/main.py#L35-L52)
...
- 初始化数据库
  ...
  - 建议在后端服务启动前执行迁移
- 启动后端服务
  - 后端监听 0.0.0.0:8000，**版本升级**：1.0.0 版本默认禁用调试模式（APP_DEBUG=false）
- 启动前端
...
- [frontend/docker-entrypoint.sh](file://frontend/docker-entrypoint.sh#L1-L11)
...
- 配置加载
  ...
  - **版本升级**：1.0.0 版本默认禁用调试模式（APP_DEBUG=false），提升安全性
- 端口与主机
...
- [backend/config.py](file://backend/config.py#L5-L59)
- [core/database.py](file://core/database.py#L11-L22)
- [frontend/vite.config.ts](file://frontend/vite.config.ts#L5-L10)
...
- [frontend/public/debug.html](file://frontend/public/debug.html#L1-L29)
- [frontend/public/test.html](file://frontend/public/test.html#L1-L25)
- [frontend/src/api/client.ts](file://frontend/src/api/client.ts#L1-L24)
...
- **版本升级**：1.0.0 版本优化了性能配置，禁用调试模式提升生产环境性能

**章节来源**
- [core/database.py](file://core/database.py#L14-L16)
...
- **版本升级**：1.0.0 版本增加了安全配置，如禁用调试模式、规范化环境变量命名

**章节来源**
- [backend/config.py](file://backend/config.py#L18-L26)
- [frontend/vite.config.ts](file://frontend/vite.config.ts#L15-L20)
- [scripts/start_agents.sh](file://scripts/start_agents.sh#L9-L34)
- [scripts/run_auto_novel.sh](file://scripts/run_auto_novel.sh#L68-L112)
- [workers/celery_app.py](file://workers/celery_app.py#L6-L25)
- [frontend/public/debug.html](file://frontend/public/debug.html#L1-L29)
- [frontend/public/test.html](file://frontend/public/test.html#L1-L25)
- [frontend/docker-entrypoint.sh](file://frontend/docker-entrypoint.sh#L1-L11)
...
## 结论
通过本指南，您可以在约 30 分钟内完成从环境准备到系统可用的全部步骤。**版本升级**：1.0.0 版本作为初始生产发布版本，提供了完整的功能集和生产就绪的安全配置。新增的调试工具进一步简化了开发和故障排查过程。建议在首次部署后，先创建一个测试小说并执行一次自动创作流程，以验证 LLM、数据库、Redis 与 Celery 的协同工作情况。后续可根据需要扩展发布平台与监控能力。
...
- 应用：APP_ENV、APP_DEBUG、APP_HOST、APP_PORT
- **新增**：前端代理：API_PROXY_TARGET（默认 http://localhost:8000）
- **版本升级**：1.0.0 版本包含完整的环境变量配置，禁用调试模式提升安全性

**章节来源**
- [.env.example](file://.env.example#L1-L21)
- [.env](file://.env#L1-L22)
- [backend/config.py](file://backend/config.py#L6-L48)
- [frontend/vite.config.ts](file://frontend/vite.config.ts#L5-L10)
...
- [backend/main.py](file://backend/main.py#L41)

### 生产环境部署建议
- **版本选择**：使用 1.0.0 版本进行生产部署
...
- [backend/config.py](file://backend/config.py#L37)
- [CHANGELOG.md](file://CHANGELOG.md#L28-L31)