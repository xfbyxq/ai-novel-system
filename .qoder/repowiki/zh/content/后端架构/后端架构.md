...
## 更新摘要
**所做更改**
- 更新应用版本信息：从 2.2.1 升级到 2.3.0，与前端版本保持一致
- 新增章节助手系统、自然语言修订、连贯性保障等新功能
- 新增章节级成本追踪与分类功能
- 新增角色自动检测及预算控制系统
...
**更新** 应用已升级至 2.3.0 版本，包含章节助手系统、自然语言修订、连贯性保障、章节级成本追踪、角色自动检测及预算控制系统等重要功能改进，以及完整的图数据库集成支持。
...
graph TB
subgraph "应用入口"
M["backend/main.py<br/>FastAPI 应用 (v2.3.0)"]
end
subgraph "路由层"
...
- **FastAPI 应用与中间件**
  ...
  - 根路径与健康检查端点提供基础服务状态。
  - **更新** 应用版本从 2.2.1 升级到 2.3.0，提供更稳定的API版本管理。
- **配置中心**
...
participant API as "FastAPI 路由 (v2.3.0)"
...
- [backend/api/v1/graph.py:105-152](file://backend/api/v1/graph.py#L105-L152)
...
- [backend/services/graph_sync_service.py:78-126](file://backend/services/graph_sync_service.py#L78-L126)
- [backend/services/graph_query_service.py:149-219](file://backend/services/graph_query_service.py#L149-L219)
...
- [backend/services/graph_sync_service.py:384-425](file://backend/services/graph_sync_service.py#L384-L425)
...
- [backend/services/graph_query_service.py:135-148](file://backend/services/graph_query_service.py#L135-L148)
- [backend/services/graph_query_service.py:149-219](file://backend/services/graph_query_service.py#L149-L219)
- [backend/services/graph_query_service.py:220-282](file://backend/services/graph_query_service.py#L220-L282)
- [backend/services/graph_query_service.py:283-319](file://backend/services/graph_query_service.py#L283-L319)
- [backend/services/graph_query_service.py:320-394](file://backend/services/graph_query_service.py#L320-L394)
- [backend/services/graph_query_service.py:395-452](file://backend/services/graph_query_service.py#L395-L452)
- [backend/services/graph_query_service.py:453-491](file://backend/services/graph_query_service.py#L453-L491)
- [backend/services/graph_query_service.py:492-522](file://backend/services/graph_query_service.py#L492-L522)
...
### 应用版本升级
- **版本标识**
  - 项目版本从 2.2.1 升级到 2.3.0，标志着系统进入更稳定的版本。
  - FastAPI 应用版本设置为 2.3.0，提供完整的 API 文档和健康检查。
  - **新增** pyproject.toml 中明确标注版本号，支持语义化版本控制。
  - **新增** CHANGELOG.md 记录详细的版本变更历史和功能更新。
  - **新增** 前端版本也同步更新到 2.3.0，确保前后端版本一致性。
- **版本管理**
  - 2.3.0版本包含章节助手系统、自然语言修订、连贯性保障、章节级成本追踪、角色自动检测及预算控制系统等新功能。
  - 2.2.1版本包含版本构建和Docker镜像更新等改进。
...
- [frontend/package.json](file://frontend/package.json#L4)
...
**章节来源**
- [CHANGELOG.md](file://CHANGELOG.md#L10)
- [CHANGELOG.md](file://CHANGELOG.md#L11)
- [CHANGELOG.md](file://CHANGELOG.md#L12)
...
**章节来源**
- [CHANGELOG.md](file://CHANGELOG.md#L17)
- [CHANGELOG.md](file://CHANGELOG.md#L18)
- [CHANGELOG.md](file://CHANGELOG.md#L19)
...
```mermaid
graph LR
API["API 路由 (v2.3.0)"] --> SGen["GenerationService"]
