...
## 更新摘要
**所做更改**
- 更新应用版本信息：从 0.1.0 升级到 1.0.0
- 新增安全增强章节：生产环境调试模式禁用
- 更新配置管理：环境变量规范化和安全配置
- 新增容器化部署支持：Docker 完整部署架构
- 更新日志配置：增强日志级别和安全处理
...
**更新** 应用已升级至 1.0.0 版本，包含完整的容器化部署支持和生产环境安全增强。
...
graph TB
subgraph "应用入口"
M["backend/main.py<br/>FastAPI 应用"]
end
subgraph "路由层"
...
- [backend/api/v1/publishing.py](file://backend/api/v1/publishing.py#L31-L31)
...
- [backend/services/encryption_service.py](file://backend/services/encryption_service.py#L10-L26)
...
- **FastAPI 应用与中间件**
  ...
  - 根路径与健康检查端点提供基础服务状态。
  - **更新** 应用版本从 0.1.0 升级到 1.0.0，提供稳定的企业级 API。
- **配置中心**
...
- [backend/config.py](file://backend/config.py#L35-L50)
- [core/database.py](file://core/database.py#L11-L35)
- [backend/dependencies.py](file://backend/dependencies.py#L12-L19)
- [backend/api/v1/generation.py](file://backend/api/v1/generation.py#L23-L104)
...
## 架构总览
下图展示请求从 FastAPI 路由进入，经服务层编排，访问数据库与外部 LLM 客户端，异步任务通过 Celery Worker 执行 GenerationService 的核心流程。**更新** 新增容器化部署架构和安全增强机制。
...
- [backend/services/encryption_service.py](file://backend/services/encryption_service.py#L27-L49)
- [core/database.py](file://core/database.py#L25-L35)
- [workers/generation_worker.py](file://workers/generation_worker.py#L26-L34)
...
- [backend/services/generation_service.py](file://backend/services/generation_service.py#L387-L564)
...
**章节来源**
- [backend/services/publishing_service.py](file://backend/services/publishing_service.py#L21-L139)
- [backend/services/publishing_service.py](file://backend/services/publishing_service.py#L144-L209)
...
- [backend/services/integration_service.py](file://backend/services/integration_service.py#L112-L292)
...
**章节来源**
- [workers/celery_app.py](file://workers/celery_app.py#L6-L25)
- [workers/generation_worker.py](file://workers/generation_worker.py#L12-L70)
...
### 应用版本升级
- **版本标识**
  - 项目版本从 0.1.0 升级到 1.0.0，标志着系统进入稳定的企业级版本。
  - FastAPI 应用版本设置为 1.0.0，提供完整的 API 文档和健康检查。
- **版本管理**
  - PyTorch 项目配置中明确标注版本号，支持语义化版本控制。
  - CHANGELOG.md 记录详细的版本变更历史和功能更新。

**章节来源**
- [pyproject.toml](file://pyproject.toml#L3)
- [backend/main.py](file://backend/main.py#L17)
...
- [.env.example](file://.env.example#L18)
- [docker-compose.yml](file://docker-compose.yml#L47)
- [CHANGELOG.md](file://CHANGELOG.md#L20)
...
```mermaid
graph LR
API["API 路由"] --> SGen["GenerationService"]
...
**章节来源**
- [backend/main.py](file://backend/main.py#L36-L53)
- [backend/services/generation_service.py](file://backend/services/generation_service.py#L198-L204)
- [backend/services/publishing_service.py](file://backend/services/publishing_service.py#L114-L138)
- [workers/generation_worker.py](file://workers/generation_worker.py#L31-L33)
- [backend/services/encryption_service.py](file://backend/services/encryption_service.py#L15-L25)
- [docker-compose.yml](file://docker-compose.yml#L13-L30)

## 结论
该后端架构以 FastAPI 为核心，通过清晰的分层与依赖注入实现高内聚低耦合；SQLAlchemy 异步 ORM 提供稳健的数据访问；Celery 异步任务体系支撑长耗时流程。服务层围绕 GenerationService、PublishingService 与 IntegrationService 构建完整的小说创作与发布闭环。**更新** 版本升级至 1.0.0，新增完整的容器化部署支持和生产环境安全增强，包括调试模式禁用、敏感数据加密、环境变量安全配置等。建议持续完善缓存与监控、压测与容量规划，以支撑更大规模的并发与更复杂的业务场景。新的架构为生产环境提供了更高的安全性、稳定性和可维护性。