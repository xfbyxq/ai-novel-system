...
## 更新摘要
**变更内容**
- 新增agent_activities表：引入Agent活动日志表，支持详细的Agent执行过程追踪
- 新增character_name_versions表：引入角色名称版本追踪表，支持角色名称变更历史管理
- 新增chapter_publishes表：完善章节发布系统，支持多平台发布状态管理
- plot_outlines表增强：新增main_plot_detailed详细主线剧情字段
- 章节表增强：章节大纲字段支持更精细的章节管理
- 枚举类型标准化：将发布相关表的状态字段转换为varchar类型
...
OutlineRefiner["大纲完善器<br/>智能完善功能"]
end
Config --> Engine
EnvVars --> Config
Compose --> Config
...
- [alembic/versions/fb6eed83562e_add_outline_dynamic_update_fields.py:21-42](file://alembic/versions/fb6eed83562e_add_outline_dynamic_update_fields.py#L21-L42)
...
- [alembic/versions/fb6eed83562e_add_outline_dynamic_update_fields.py:1-43](file://alembic/versions/fb6eed83562e_add_outline_dynamic_update_fields.py#L1-L43)
...
**图表来源**
- [core/models/novel.py:55-64](file://core/models/novel.py#L55-L64)
- [core/models/plot_outline.py:75-88](file://core/models/plot_outline.py#L75-L88)
...
**图表来源**
- [backend/services/generation_service.py:799-801](file://backend/services/generation_service.py#L799-L801)
...
- **平台扩展支持**：新增抖音等爬虫平台的支持。
- **动态大纲更新机制**：通过update_history和version字段实现大纲的智能更新和版本管理。
- **智能更新系统**：OutlineDynamicUpdater提供基于章节写作进度的大纲自动更新功能。
- **章节配置系统**：通过chapter_config字段支持灵活的章节数设置。
- **详细主线剧情**：通过main_plot_detailed字段提供增强版主线剧情描述。
- **智能完善功能**：OutlineRefiner提供基于AI的大纲智能完善和版本对比。
- **Agent活动追踪**：通过agent_activities表实现详细的Agent执行过程追踪。
- **章节发布管理**：通过chapter_publishes表实现多平台发布状态管理。
- **系统监控能力**：支持完整的系统状态监控和任务历史追踪。

通过合理的实体关系、外键约束与索引策略，兼顾了数据一致性与查询效率。配合异步连接池与事务生命周期管理，以及 SSL 连接禁用配置，能够满足高并发场景下的稳定性与可维护性需求。**新增** 的动态更新功能使得小说创作过程更加智能化，能够根据实际写作进度自动调整和完善大纲结构。**新增** 的智能完善功能进一步提升了大纲的质量和完整性，为创作者提供了强大的AI辅助工具。**新增** 的Agent活动追踪系统为整个小说生成系统的监控和调试提供了强有力的技术支撑，使得系统的行为更加透明和可控。
