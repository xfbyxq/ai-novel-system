...
async def demo_outline_enhancement():
        ...
        "volumes": [
            {
                ...
                "key_events": [
                    {"chapter": 3, "event": "获得神秘的机械戒指", "impact": "开启代码修仙之路"},
                    {"chapter": 12, "event": "第一次成功运行代码法术", "impact": "证明了自己的价值"},
                    {"chapter": 20, "event": "收服第一个代码精灵", "impact": "实力大增"}
                ]
            }
        ],
        "key_turning_points": [
    ...
    try:
        ...
        for i in range(2):
            ...
            print(f"  迭代 {i+1}: 质量={quality_score:.1f}, 一致性={consistency_score:.1f}, 继续={should_continue}")
    ...
    try:
        ...
        print("✅ 大纲协作完善流程模拟完成")
        print(f"📊 最终一致性评分: {enhancement_result['consistency_report']['consistency_score']}")
        print(f"📊 最终质量评分: {enhancement_result['quality_report']['final_quality_score']}")
