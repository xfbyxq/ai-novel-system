...
class CharacterProfile:
    ...
    def from_character_data(cls, character_data: Dict[str, Any]) -> "CharacterProfile":
        ...
        # 尝试从不同字段提取核心动机
        core_motivation = (
            character_data.get("core_motivation", "")
            or character_data.get("motivation", "")
            or character_data.get("goal", "")
            or "未定义"
        )

        # 尝试提取行为准则
        personal_code = (
            character_data.get("personal_code", "")
            or character_data.get("principle", "")
            or character_data.get("code", "")
            or "无明显准则"
        )
        ...
        personality_traits = (
            ...
            or character_data.get("traits", [])
            or character_data.get("personality", "").split(",")
            or ["未定义"]
        )
...
"""
    ...
    def __init__(self, character_profile: CharacterProfile):
        ...
        logger.info(
            f"CharacterConsistencyTracker initialized for {character_profile.name}"
        )
        ...
        if any(word in decision_lower for word in ["战斗", "逃避", "谈判", "妥协"]):
            if "conflict_response" not in self.behavioral_patterns:
                self.behavioral_patterns["conflict_response"] = BehavioralPattern(
                    pattern_type="conflict_response", description=""
                )
            ...
            if len(pattern.examples) > 0:
                pattern.description = (
                    f"倾向于{decision.decision.split()[-1]}的方式解决冲突"
                )
        ...
        if any(word in decision_lower for word in ["决定", "选择", "放弃", "坚持"]):
            if "decision_making" not in self.behavioral_patterns:
                self.behavioral_patterns["decision_making"] = BehavioralPattern(
                    pattern_type="decision_making", description=""
                )
    ...
    def validate_action(
        ...
        """
        logger.info(
            f"Validating action for {self.profile.name}: {proposed_action[:50]}..."
        )

        validation = ConsistencyValidation()

        # 维度 1：动机一致性
        motivation_result = self._check_motivation_alignment(proposed_action, context)
        ...
        # 综合判断
        validation.passed = validation.overall_score >= 0.7 and not any(
            issue["severity"] == "critical" for issue in validation.issues
        )
    ...
    def _check_motivation_alignment(self, action: str, context: str) -> Dict[str, Any]:
        ...
        matches = sum(
            ...
            if kw in action_lower or kw in context.lower()
        )
        ...
        return {
            ...
            "score": min(1.0, score),
            "reason": (
                "行为与核心动机一致" if score >= 0.6 else "行为与核心动机关联不明确"
            ),
        }
    ...
    def _check_code_adherence(self, action: str) -> Dict[str, Any]:
        ...
        # 检查是否明显违背准则
        violating_keywords = ["伤害无辜", "背叛", "欺骗", "偷窃", "逃跑"]
        ...
        return {"adherent": True, "score": 1.0, "reason": "行为未违背已知准则"}
    ...
    def _check_personality_consistency(
        ...
        return {
            ...
            "score": max(0.0, score),
            "reason": ("行为与性格一致" if score >= 0.6 else "行为与某些性格特质不符"),
        }
    ...
    def _generate_suggestions(self, validation: ConsistencyValidation) -> List[str]:
        ...
        if validation.personality_consistency < 0.6:
            ...
            suggestions.append(f"建议调整行为方式以体现{traits_str}的特点")

        if validation.historical_consistency < 0.8:
            suggestions.append("建议说明角色为何改变之前的决定")
