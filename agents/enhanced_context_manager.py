...
class RecentChapter:
    ...
    def to_prompt(self) -> str:
        ...
        if self.character_changes:
            changes = "; ".join(f"{k}: {v}" for k, v in list(self.character_changes.items())[:3])
    ...
    def _build_critical_layer(
        ...
        for foreshadow in foreshadowings:
            ...
            if importance >= 7 and urgency >= 3:
                elements.append(CriticalElement(
                    ...
                    metadata={"expected_resolve": foreshadow.get("expected_resolve_chapter")}
                ))
            # 中等重要性 + 超 5 章 = 建议携带
    ...
    def _build_recent_layer(
        ...
        for ch in range(start_chapter, chapter_number):
            ...
            recent = RecentChapter(
                ...
                word_count=len(chapter_contents.get(ch, "")) if ch in chapter_contents else 0
            )
    ...
    def _build_historical_layer(
        ...
        if volume_summaries:
            for vol_num, vol_data in sorted(volume_summaries.items()):
                ...
                if vol_end < cold_end:
                    historical_indices.append(HistoricalIndex(
