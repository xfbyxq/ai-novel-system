...
class ContinuityAssuranceIntegration:
    ...
    async def enforce_continuity(
        ...
        """
        logger.info(
            f"开始连贯性保障流程：小说={novel_id}, 章节={chapter_number}"
        )
        ...
        for attempt in range(max_regeneration_attempts + 1):
            ...
            # 检查是否通过验证
            if report.quality_score >= min_quality_score and not report.needs_regeneration:
                logger.info(
...
"""
    ...
    def _create_transition_record(
        ...
        if validation_report.quality_score >= 90:
            final_decision = "直接采用"
            modification_notes = f"质量评分 {validation_report.quality_score:.1f}，无需修改"
    ...
    async def _save_transition_record(
        self,
