...
class ChapterTransition:
    ...
    def __post_init__(self):
        ...
        if self.final_decision not in valid_decisions:
            raise ValueError(f"Invalid decision: {self.final_decision}. Must be one of {valid_decisions}")
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "ChapterTransition":
        ...
        return cls(
            ...
            inferred_constraints=[
                ContinuityConstraint.from_dict(c) for c in data.get("inferred_constraints", [])
            ],
