...
class ThemeDefinition:
    ...
    def from_novel_data(cls, novel_data: Dict[str, Any]) -> "ThemeDefinition":
        ...
        if not central_question:
            # 从主角目标推断
            main_plot = plot_outline.get("main_plot", {}) if isinstance(plot_outline, dict) else {}
...
"""
    ...
    def _calculate_main_plot_progress(
        ...
        return {
            ...
            "analysis": "。".join(analysis_parts) if analysis_parts else "本章对主线推进作用不明显"
        }
    ...
    def _analyze_subplots(self, chapter_plan: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for subplot in subplots:
            ...
            if any(
                ...
                for keyword in [self.theme.core_theme, self.theme.main_conflict.split(" vs ")[0]]
            ):
                relevance_score = 0.8
    ...
    def _calculate_overall_score(self, report: ThemeConsistencyReport) -> float:
        ...
        score = (
            ...
            report.character_motivation_alignment * weights["character_motivation_alignment"] +
    ...
    def _generate_suggestions(
        ...
        if report.main_plot_advancement < 6.0:
            suggestions.append(
                "建议增加与核心问题直接相关的事件，明确推进主线剧情"
            )
        
        if report.character_motivation_alignment < 6.0:
            suggestions.append(
                "建议为角色行为提供更清晰的动机说明，确保符合角色设定"
            )
        ...
        if report.theme_expression < 6.0:
            suggestions.append(
                f"建议在章节中深化「{self.theme.core_theme}」主题的表达"
            )
