...
class PreventionReport:
    ...
    def has_issues(self) -> bool:
        """是否有问题"""
        return (
            len(self.ignored_constraints) > 0 or
            len(self.conflicts) > 0 or
            len(self.missing_progress) > 0
        )
...
"""
    ...
    async def check_chapter_plan(
        ...
        # 1. 检查约束回应
        constraint_check = self._check_constraint_response(
            chapter_plan, constraints
        )
        report.ignored_constraints = constraint_check["ignored"]
        report.constraint_response_score = constraint_check["score"]
        
        # 2. 检查情节一致性
        consistency_check = self._check_plot_consistency(
            chapter_plan, previous_chapter
        )
    ...
    def _check_plot_consistency(
        ...
        if prev_location and plan_location:
            if prev_location != plan_location and not self._is_location_transition_valid(
    ...
    def _check_plot_progress(
        ...
        if not main_events:
            missing.append(MissingProgress(
    ...
    def _is_state_transition_smooth(
        self,
        prev_ending: str,
        plan_start: str
    ) -> bool:
    ...
    def _plan_addresses_conflict(
        self,
        chapter_plan: Dict[str, Any],
        conflict_constraint: ContinuityConstraint
    ) -> bool:
        """检查策划是否回应了冲突"""
        chapter_plan_text = json.dumps(chapter_plan, ensure_ascii=False).lower()
    ...
    async def suggest_plan_fixes(
        ...
        for conflict in report.conflicts:
            if conflict.conflict_type == "location":
                ...
                if "opening" not in fixed_plan:
                    fixed_plan["opening"] = ""
                fixed_plan["opening"] += f"从{conflict.previous_chapter_element.split('：')[-1]}来到{conflict.current_plan_element.split('：')[-1]}..."
        ...
        logger.info(f"Suggested {len(fixed_plan.get('constraint_responses', []))} fixes")
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_issues_found": sum(len(r.ignored_constraints) + len(r.conflicts) + len(r.missing_progress) for r in self.check_history)
        }
