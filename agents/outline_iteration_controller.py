...
class OutlineIterationController:
    ...
    def log_iteration(
        ...
        if combined_score > self.best_score:
            ...
            logger.info(f"[OutlineIterationController] 发现更优版本 (score={combined_score:.2f})")
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "final_consistency_score": consistency_scores[-1] if consistency_scores else 0.0,
            "quality_progression": quality_scores,
            "consistency_progression": consistency_scores,
            "cumulative_cost": round(self.cumulative_cost, 4),
            "converged": bool(
                quality_scores and consistency_scores and
                quality_scores[-1] >= self.quality_threshold and
                consistency_scores[-1] >= self.consistency_threshold
            ),
    ...
    async def optimize_outline_iteratively(
        ...
        while self.should_continue(
            ...
            if optimization_suggestions:
                ...
                # 记录变更
                changes_made = self._summarize_changes(current_outline, optimized_outline)
                issues_resolved = self._identify_resolved_issues(optimization_suggestions)
                
                current_outline = optimized_outline
            
            # 记录本次迭代
            remaining_issues = self._identify_remaining_issues(quality_result, consistency_result)
    ...
    def _generate_optimization_plan(
        ...
        # 基于一致性检查的建议
        if consistency_result.get("consistency_score", 5.0) < self.consistency_threshold:
            ...
            if not extended_analysis.get("character_mapping", {}).get("well_distributed", False):
                suggestions.append({
    ...
    async def _apply_optimizations(
        ...
        for suggestion in suggestions:
            ...
            if suggestion_type == "worldview_integration":
                ...
                optimized_outline["character_development_plan"].append("优化角色戏份分配")
        
        return optimized_outline
    
    def _summarize_changes(
        self, 
        old_outline: Dict[str, Any], 
