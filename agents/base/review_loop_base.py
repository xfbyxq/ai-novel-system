...
class IssueTracker:
    ...
    def update_round(
        ...
        for issue_dict in current_issues:
            ...
            if matched:
                ...
                # 新问题
                record = IssueRecord(
                    id=self._generate_id(issue_dict.get("area", ""), issue_dict.get("description", "")),
        ...
        for record in self._records:
            if record.id not in matched_record_ids and record.status in ("open", "recurring"):
    ...
    def format_for_reviewer(self, max_chars: int = 1000) -> str:
        ...
        if active:
            ...
            for r in sorted_active:
                area_tag = f"[{r.area}] " if r.area else ""
                persist_info = f"已持续{r.last_seen_round - r.first_seen_round + 1}轮" if r.status == "recurring" else ""
    ...
    def _extract_issues(
        ...
        def add_issue(area: str, desc: str, severity: str = "medium", suggestion: str = ""):
...
class ReviewProgressSummary:
    ...
    def format_for_reviewer(self, max_chars: int = 800) -> str:
        ...
        change_text = f"{'+'if total_change >= 0 else ''}{total_change:.1f}" if len(self._scores) > 1 else ""
        lines.append(f"评分趋势：{scores_text}（{self.score_trend}{', ' + change_text if change_text else ''}）")
