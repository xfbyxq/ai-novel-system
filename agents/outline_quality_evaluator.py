...
class OutlineQualityEvaluator:
    ...
    async def _evaluate_all_dimensions(
        ...
        # 结构完整性评估
        dimension_scores["structure_completeness"] = await self._evaluate_structure_completeness(outline)
        
        # 世界观一致性评估
        dimension_scores["setting_consistency"] = await self._evaluate_setting_consistency(outline, world_setting)
        
        # 角色连贯性评估
        dimension_scores["character_coherence"] = await self._evaluate_character_coherence(outline, characters)
        
        # 张力节奏评估
        dimension_scores["tension_management"] = await self._evaluate_tension_management(outline)
        
        # 逻辑连贯性评估
        dimension_scores["logical_flow"] = await self._evaluate_logical_flow(outline)
        
        # 创意新颖性评估
        dimension_scores["innovation_factor"] = await self._evaluate_innovation_factor(outline)
    ...
    async def _evaluate_character_coherence(
        ...
        try:
            ...
            # 检查主要角色的存在感
            main_characters = [c for c in characters if c.get("importance", "supporting") == "main"]
            ...
            relationship_score = sum(1 for keyword in relationship_keywords if keyword in outline_text)
    ...
    async def _evaluate_tension_management(self, outline: Dict[str, Any]) -> float:
        ...
        try:
            ...
            if climax_chapter and isinstance(climax_chapter, int):
                ...
                climax_position = climax_chapter / total_chapters if total_chapters > 0 else 0
                # 高潮应在70%-90%位置
    ...
    async def _evaluate_logical_flow(self, outline: Dict[str, Any]) -> float:
        ...
        try:
            ...
            # 检查逻辑连接词
            logical_connectors = ["但是", "然而", "不过", "尽管", "虽然", "而且", "此外"]
            connector_score = sum(1 for word in logical_connectors if word in outline_text)
