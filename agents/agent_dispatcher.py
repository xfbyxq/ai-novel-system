...
class AgentDispatcher:
    ...
    def set_use_scheduled_agents(self, use_scheduled: bool):
        ...
        logger.info(
            f"🎮 Agent调度器模式设置为: {'基于调度器' if use_scheduled else 'CrewAI风格'}"
        )
    ...
    async def run_planning(
        ...
        if self.use_scheduled_agents:
            return await self._run_planning_with_scheduled_agents(
                novel_id, task_id, **kwargs
            )
        else:
    ...
    async def _run_planning_with_scheduled_agents(
        ...
        try:
            ...
            if (
                not market_task_result
                or market_task_result.status != TaskStatus.COMPLETED
            ):
            ...
            if (
                not content_task_result
                or content_task_result.status != TaskStatus.COMPLETED
            ):
            ...
            logger.warning(
                "⚠️  调度器当前仅支持市场分析，降级到 CrewManager 执行完整企划"
            )
            return await self._run_planning_with_crew_manager(novel_id, **kwargs)
    ...
    async def run_chapter_writing(
        ...
        **kwargs,
        ...
        """
        if self.use_scheduled_agents:
            return await self._run_chapter_writing_with_scheduled_agents(
                novel_id, task_id, chapter_number, volume_number, **kwargs
            )
        else:
            return await self._run_chapter_writing_with_crew_manager(
                novel_id, chapter_number, volume_number, **kwargs
            )

    async def _run_chapter_writing_with_scheduled_agents(
        self,
        novel_id: UUID,
        task_id: UUID,
        chapter_number: int,
        ...
        **kwargs,
        ...
        """
        logger.info(
            f"🎮 使用基于调度器的Agent系统执行第{chapter_number}章写作 - 小说ID: {novel_id}"
        )
    ...
    async def _run_chapter_writing_with_crew_manager(
        ...
        """
        logger.info(
            f"🎮 使用CrewAI风格系统执行第{chapter_number}章写作 - 小说ID: {novel_id}"
        )
    ...
    async def run_batch_writing(
        ...
        **kwargs,
        ...
        """
        logger.info(
            f"🎮 执行批量写作 - 小说ID: {novel_id}, 章节范围: {from_chapter}-{to_chapter}"
        )
    ...
    async def _run_chapter_writing_with_scheduled_agents(
        ...
        **kwargs,
        ...
        try:
            ...
            if (
                not writing_task_result
                or writing_task_result.status != TaskStatus.COMPLETED
            ):
                logger.error("❌ 写作任务失败")
                return await self._run_chapter_writing_with_crew_manager(
                    novel_id, chapter_number, volume_number, **kwargs
            ...
            editing_task = AgentTask(
                ...
                input_data={
                    ...
                    "chapter_number": chapter_number,
                    "chapter_title": writing_result.get(
            ...
            if (
                not editing_task_result
                or editing_task_result.status != TaskStatus.COMPLETED
            ):
                logger.error("❌ 编辑任务失败")
                return await self._run_chapter_writing_with_crew_manager(
                    novel_id, chapter_number, volume_number, **kwargs
            ...
            final_result = {
                ...
                "edited_content": editing_result.get(
                    "edited_content", writing_result.get("content", "")
                ),
                "final_content": editing_result.get(
                    "edited_content", writing_result.get("content", "")
                ),
                "continuity_report": {},
                "quality_score": 0,
