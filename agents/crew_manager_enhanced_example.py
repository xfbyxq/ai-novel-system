...
class EnhancedCrewManager:
    ...
    async def run_writing_phase(
        ...
        prep_result = await self._prepare_chapter_generation(
            chapter_number=chapter_number, volume_number=volume_number, **kwargs
        )

        # ========== 步骤 2：策划阶段 ==========
        chapter_plan = await self._run_enhanced_planning(
            chapter_number=chapter_number, prep_result=prep_result, **kwargs
        )
        ...
        if not review_result.passed:
            ...
            chapter_plan = await self._fix_chapter_plan(
                original_plan=chapter_plan, review_result=review_result
            )

        # ========== 步骤 4：生成阶段 ==========
        writing_result = await self._generate_chapter_content(
            chapter_plan=chapter_plan, prep_result=prep_result, **kwargs
        )
    ...
    async def _run_enhanced_planning(
        ...
        enhanced_prompt = self._build_enhanced_planner_prompt(
            prep_result=prep_result, **kwargs
        )
...
""")
        ...
        if review_result.suggestions:
            ...
            for suggestion in review_result.suggestions[:3]:  # 最多添加 3 个
                fixed_plan["additional_events"].append(
                    {"type": "suggestion", "description": suggestion}
                )
    ...
    async def _generate_chapter_content(
        ...
        return {"content": chapter_content, "word_count": len(chapter_content)}
...
async def example_usage():
    ...
    # 创建增强版 CrewManager
    crew_manager = EnhancedCrewManager(novel_id="novel-001", novel_data=novel_data)
    ...
    result = await crew_manager.run_writing_phase(
        ...
        previous_chapter={"ending_state": "第 2 章结尾", "chapter_number": 2},
    )
